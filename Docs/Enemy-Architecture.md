...
| 종류별 특수 능력(실드/내성) (4장) | `IDamageable`+`DamageType`, `EnemyAbility`(확장점), 감속/도트 피동 상태 |
| 처치 시 재화 드랍 (7장) | `EnemyData` 보상 필드 → `EnemyManager.OnEnemyKilled` → `EnemyRewardDropper` |
...
속성별 로직은 구현됨: 내성은 `Enemy.ApplyDamage`의 배수(`EnemyData`), 도트/감속은 `Enemy.ApplyDoT`/`ApplySlow`가 처리한다.
...
| `energyDrop` / `goldDrop` | 처치 보상. `EnemyRewardDropper`가 소비(에너지=즉시, 골드=클리어 시 뱅킹) |
...
| `ApplySlow(mul, dur)` | 이동속도 감속 부여(프리즈 터렛). 더 강한 감속(작은 배수) 채택·지속시간 갱신 |
| `ApplyDoT(dps, dur, type)` | 초당 지속 피해 부여(파이어 터렛). 더 강한 도트 채택·지속시간 갱신 |
...
**피동 상태(감속·도트) 처리:** `ApplySlow`/`ApplyDoT`는 터렛이 **한 번 스탬프만 찍고**, 만료는 적이
스스로 처리한다(`AddShield`와 같은 계층). 감속은 `Update`의 이동 속도에 배수로 곱해지고 만료 시각이
지나면 자동 해제된다. 도트는 `TickDoT`가 1초 주기로 피해를 주되 **방어력·보호막은 무시하고 속성 내성만
적용**한다(공병의 화염 내성이 도트에도 걸림). 피격 경로는 `ApplyDamage(…, ignoreArmor, ignoreShield)`로
통일해 일반 타격·도트가 같은 코드를 공유한다.
...
| `OnEnemyKilled` | 처치 통지 — `EnemyRewardDropper`가 보상 드랍 구독 |
...
### 4.11 `EnemyRewardDropper` (MonoBehaviour)

적 처치 보상 드랍 훅(CLAUDE.md 7장). `EnemyManager.OnEnemyKilled`를 **구독만** 해 처치 시 재화를
지급한다(`EnemyManager`/`Enemy`를 오염시키지 않도록 `BaseCore`와 동일하게 별도 컴포넌트로 분리).
씬에 1개 두며(EnemyManager 오브젝트에 부착해도 됨), 소스가 늦게 생성돼도 `OnEnable`/`Start`에서 재시도 구독한다.

두 재화의 성격이 달라 처리 방식이 다르다:

| 재화 | 처리 |
|---|---|
| 에너지(스테이지 한정 전술 재화) | 처치 **즉시** `PlayerEconomy.Add(energyDrop)`로 지갑에 지급 |
| 골드(영구 성장 재화) | `PendingGold`에 **세션 누적만** → `GameManager.OnGameEnded==Cleared`에서만 `PlayerProgress.AddGold`로 확정. 패배 시 폐기 |

골드를 즉시 영속화하지 않고 클리어 시에만 확정하는 것은 "클리어 실패 시 보상 없음"(CLAUDE.md 7장)과
일관되게 하기 위함이다. 클리어 시 별점 보상(`StageProgressRecorder`)과 같은 종료 이벤트에서 함께 확정된다.
...
- [x] **터렛 연동**: `FindNearest` 타게팅 + 5종(단일/광역/감속/도트/에너지 생산) 배치·조준·발사·철거 구현.
      상세·나머지 계획은 [`Turret-Architecture.md`](./Turret-Architecture.md) §6 참고.

...
- **기지 생성:** 타일을 모두 채운 뒤 `BaseCell`(정중앙 Base 타일) 위에 기지 프리팹(`basePrefab` 또는 `Resources/Prefabs/Base`)을 세운다. 메시 밑면 중심을 타일 윗면에 맞춰 정규화 배치하고, **그 셀의 타일 오브젝트 자식**으로 붙여 맵 회전 시 위치·방향이 함께 회전한다(터렛 동반 회전과 동일 규약). 프리팹이 없으면 Cube 폴백. 생성된 오브젝트는 `BaseInstance`로 노출.
- 회전은 다루지 않음(평평한 계층). 재빌드용 `ClearTiles()` 제공(기지는 타일의 자식이라 함께 정리).
...
- [x] `MapBuilder`: StageData → 씬 타일 생성 + Base 타일 위 기지 오브젝트 생성(회전 동반)
